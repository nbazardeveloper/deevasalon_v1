...
import { t as require_jsx_runtime } from "./jsx-runtime-BD80muvF.js";
...
var QueryObserver = class extends Subscribable {
	constructor(client, options) {
		...
		if (!this.options.experimental_prefetchInRender) this.#currentThenable.reject(/* @__PURE__ */ new Error("experimental_prefetchInRender feature flag is not enabled"));
		this.bindMethods();
...
function dehydrateQuery(query, serializeData, shouldRedactErrors) {
	return {
		...
		...query.state.status === "pending" && { promise: query.promise?.then(serializeData).catch((error) => {
...
function hydrate(client, dehydratedState, options) {
	...
	queries.forEach(({ queryKey, state, queryHash, meta, promise, dehydratedAt }) => {
		...
		if (promise && !existingQueryIsPending && !existingQueryIsFetching && (dehydratedAt === void 0 || dehydratedAt > query.state.dataUpdatedAt)) query.fetch(void 0, { initialPromise: Promise.resolve(promise).then(deserializeData) });
	});
}
//#endregion
//#region node_modules/@tanstack/query-core/build/modern/streamedQuery.js
function streamedQuery({ queryFn, refetchMode = "reset", maxChunks }) {
	return async (context) => {
		...
		for await (const chunk of stream) {
			...
			if (!isRefetch || refetchMode !== "replace") context.client.setQueryData(context.queryKey, (prev = []) => {
...
function useMutationState(options = {}, queryClient) {
	...
	if (!result.current) result.current = getResult(mutationCache, options);
	import_react.useEffect(() => {
		optionsRef.current = options;
	});
